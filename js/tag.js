	...
	$.getJSON(OC.linkTo('facefinder', 'ajax/tag.php')+'?image='+img, function(data) {
		$.each(data,function(index_tag,data){
			...
			$('#taggs div.tag img').click(function(){
				...
				$("#photo").append('<div class="draggable_fix" style="position: absolute; top: '+y1+'px; left: '+x1+'px;"><div class="draggable_tag"><img alt="" src="'+OC.linkTo('facefinder', 'img/delete.png')+'" name="'+data.name+' '+data.tag+'">'+data.tag+'</div></input></div>');
		...
		// alert(PosX+' '+PosY);
		 ...
		 $("#photo .draggable input").keyup(function(e){
			 ...
			 if ( e.keyCode== 13){
				 ...
				 var image=$('#photoview img').attr("name");
				 var tag=$(this).val();
				 var x=(pos[0]/document.getElementById("img_img").offsetWidth);
		 		 var y=(pos[1]/document.getElementById("img_img").offsetHeight);
					$.getJSON(OC.linkTo('facefinder', 'ajax/inserttagposition.php')+"?image="+image+"&tag="+tag+"&x1="+x+"&x2="+(pos[0]+100)+"&y1="+y+"&y2="+(pos[1]+100), function(data) {
						$("#taggs div").remove();
						$("#photo div").remove();
						Tag.getTag(image);
					});
