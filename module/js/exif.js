

function exif() {
	this.load=function(image){
		if(!$('#tool_righte').is(":visible")){
			$('#tool_righte').show();
		}
		$("#tool_righte .tool.Exif .tool_items p").remove();
		$.getJSON(OC.linkTo('facefinder', 'ajax/EXIF.php')+'?image='+image, function(data) {
			if (data.status == 'success'){
					$.each(data.data,function(index_tag,data){
						$("#tool_righte .tool.Exif .tool_items ul").append('<li>'+data.name+':'+data.tag+'</li>');
					});
			}
		});
	};
	
}




$(document).ready(function() {
	$("#tool_righte").append('<div class="tool Exif"><div class="tool_title"><i class="icon-info-sign"></i>Exif</div><div class="tool_items"><ul></ul></div></div>');
});
