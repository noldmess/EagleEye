...
class FaceDetaction_Module implements OCA\FaceFinder\MapperInterface{
	...
	public static  function insert($class){
		...
		OCP\BackgroundJob::addQueuedTask('facefinder','OCA\FaceFinder\HooksHandlers','startBackgroundJob',json_encode(array("class"=>"FaceDetaction_Module","id"=>$class->getForingkey(),"paht"=>$class->getPaht())));
	}
				...
				foreach ($newClass->makeFaceTagArray() as  $section) {
					...
					if($face['class']>41 && $face['threshold']<80){
						...
						if($tag['tag_id']!==null){
							...
							self::insertFacePhoto($tag['tag_id'],$newClass,$imgToSava."-".$facecount.".png",$face['class'],$section['y1'],$section['x1'],$section['y2'],$section['x2']);
						}else{
							OCP\Util::writeLog("facefinder","not found",OCP\Util::DEBUG);
							self::insertFacePhoto(null,$newClass,$imgToSava."-".$facecount.".png",null,$section['x1'],($section['x1']-$section['x2']),$section['y1'],($section['y1']-$section['y2']));
						}
